<template>
	<div class="main-content">
		...
		<template v-if="showFlag">
			...
			<!-- <div> -->
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='false' prop="zhongqidabian" label="中期答辩">
						<template slot-scope="scope">
							<el-button v-if="scope.row.zhongqidabian" type="text" size="small" @click="download($base.url+scope.row.zhongqidabian)">下载</el-button>
                            <span v-else >无</span>
						</template>
					</el-table-column>
					...
					<el-table-column :resizable='true' :sortable='false' v-if="isAuth('zhongqidabian','审核')" prop="sfsh" label="审核">
						<template slot-scope="scope">
		...
		<jietidabian-cross-add-or-update v-if="jietidabianCrossAddOrUpdateFlag" :parent="this" ref="jietidabianCrossaddOrUpdate"></jietidabian-cross-add-or-update>
...
import jietidabianCrossAddOrUpdate from "../jietidabian/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      jietidabianCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    jietidabianCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
	if(crossOptAudit=='是'&&row.sfsh!='是') {
      ...
      this.jietidabianCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','zhongqidabian');
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.jietidabianCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.jietidabianCrossaddOrUpdate.init(row.id,type);
      });
    },
    ...
    shDialog(row){
      ...
      if(row){
        this.shForm = {
          ...
          zhongqidabian: row.zhongqidabian,
          sfsh: row.sfsh,
          shhf: row.shhf,
          id: row.id
        }
