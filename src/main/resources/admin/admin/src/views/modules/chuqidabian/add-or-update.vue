	...
	<div class="addEdit-block" style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 20px 0"}' class="upload" v-if="type!='info'&& !ro.dabianwenjian" label="答辩文件" prop="dabianwenjian">
					<file-upload
						tip="点击上传答辩文件"
						action="file/upload"
						:limit="1"
						:multiple="true"
						:fileUrls="ruleForm.dabianwenjian?ruleForm.dabianwenjian:''"
						@change="dabianwenjianUploadChange"
					></file-upload>
				</el-form-item>  
				<el-form-item :style='{"width":"48%","margin":"0 0 20px 0"}' v-else-if="ruleForm.dabianwenjian" label="答辩文件" prop="dabianwenjian">
					<el-button :style='{"border":"1px solid #aaa","cursor":"pointer","padding":"0 12px","margin":"0 20px 0 0","color":"#333","minWidth":"80px","outline":"none","borderRadius":"4px","background":"linear-gradient(200deg, rgba(216,223,236,1) 0%, rgba(247,249,251,1) 50%, rgba(210,217,230,1) 100%)","width":"auto","lineHeight":"36px","fontSize":"14px","height":"36px"}' type="text" size="small" @click="download($base.url+ruleForm.dabianwenjian)">下载</el-button>
				</el-form-item>
				<el-form-item :style='{"width":"48%","margin":"0 0 20px 0"}' v-else-if="!ruleForm.dabianwenjian" label="答辩文件" prop="dabianwenjian">
...
export default {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='dabianwenjian'){
							this.ruleForm.dabianwenjian = obj[o];
							this.ro.dabianwenjian = true;
							continue;
						}
				}
	...
	if(this.ruleForm.dabianwenjian!=null) {
		this.ruleForm.dabianwenjian = this.ruleForm.dabianwenjian.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `chuqidabian/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.chuqidabianCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    dabianwenjianUploadChange(fileUrls) {
	this.ruleForm.dabianwenjian = fileUrls;
    },
  }
};
</script>
