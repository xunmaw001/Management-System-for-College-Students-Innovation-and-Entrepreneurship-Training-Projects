	...
	<div class="addEdit-block" style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 20px 0"}' class="upload" v-if="type!='info'&& !ro.jietidabian" label="结题答辩" prop="jietidabian">
					<file-upload
						tip="点击上传结题答辩"
						action="file/upload"
						:limit="1"
						:multiple="true"
						:fileUrls="ruleForm.jietidabian?ruleForm.jietidabian:''"
						@change="jietidabianUploadChange"
					></file-upload>
				</el-form-item>  
				<el-form-item :style='{"width":"48%","margin":"0 0 20px 0"}' v-else-if="ruleForm.jietidabian" label="结题答辩" prop="jietidabian">
					<el-button :style='{"border":"1px solid #aaa","cursor":"pointer","padding":"0 12px","margin":"0 20px 0 0","color":"#333","minWidth":"80px","outline":"none","borderRadius":"4px","background":"linear-gradient(200deg, rgba(216,223,236,1) 0%, rgba(247,249,251,1) 50%, rgba(210,217,230,1) 100%)","width":"auto","lineHeight":"36px","fontSize":"14px","height":"36px"}' type="text" size="small" @click="download($base.url+ruleForm.jietidabian)">下载</el-button>
				</el-form-item>
				<el-form-item :style='{"width":"48%","margin":"0 0 20px 0"}' v-else-if="!ruleForm.jietidabian" label="结题答辩" prop="jietidabian">
...
export default {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='jietidabian'){
							this.ruleForm.jietidabian = obj[o];
							this.ro.jietidabian = true;
							continue;
						}
				}
	...
	if(this.ruleForm.jietidabian!=null) {
		this.ruleForm.jietidabian = this.ruleForm.jietidabian.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jietidabian/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jietidabianCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    jietidabianUploadChange(fileUrls) {
	this.ruleForm.jietidabian = fileUrls;
    },
  }
};
</script>
